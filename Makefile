...
BINDIST_EXTRAS += README.md data doc README_LUAEX.txt luaex_doc
...
LUA_AUTOGEN_SOURCES = $(wildcard $(LUASRC_DIR)/*.cpp)
_LUA_AUTOGEN_OBJS = $(LUA_AUTOGEN_SOURCES:$(LUASRC_DIR)/%.cpp=$(LUA_DIR)/%.o)
LUA_AUTOGEN_OBJS = $(sort $(patsubst %,$(ODIR)/%,$(_LUA_AUTOGEN_OBJS)))
...
$(TARGET): $(OBJS) $(LUA_AUTOGEN_OBJS)
	+$(LD) $(W32FLAGS) -o $(TARGET) $(OBJS) $(LUA_AUTOGEN_OBJS) $(LDFLAGS)
ifdef RELEASE
  ifndef DEBUG_SYMBOLS
    ifneq ($(BACKTRACE),1)
	$(STRIP) $(TARGET)
...
$(BUILD_PREFIX)$(TARGET_NAME).a: $(OBJS) $(LUA_AUTOGEN_OBJS)
	$(AR) rcs $(BUILD_PREFIX)$(TARGET_NAME).a $(filter-out $(ODIR)/main.o $(ODIR)/messages.o,$(OBJS)) $(LUA_AUTOGEN_OBJS)
